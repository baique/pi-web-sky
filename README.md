...
Pi 编程智能体的本地浏览器界面：查找与继续会话、运行智能体、配置模型与资源、浏览项目文件。本发行版在原生 [pi-web](https://github.com/earendil-works/pi) 之上重做界面并扩展工作流能力（会话看板、内置终端、消息钉等），与官方版共用本机 `~/.pi/agent` 数据，二者可随时切换。
...
首次使用先在 **Models** 面板登录或配置 API Key。

> **安全**：默认只监听本机 `127.0.0.1`。如要对内网开放，请用 `PI_WEB_PASSWORD` 开启 Basic Auth，并在 HTTPS/VPN 环境使用——对外暴露意味着授权该进程执行高权限操作。

## 功能亮点

### 会话看板（任务即看板）

把会话铺在无限画布上管理，是本发行版的核心工作台：

- **任务就是看板**：点任务行直接进入它的画布，任务内会话自动成卡、新会话自动补卡，卡片位置刷新不丢；任务改名看板同步改名，删任务连带删看板。
- **卡片即工作台**：收合卡显示状态/标题/最后回复，展开卡直接在画布上聊天、看状态、盯统计，展开后仍可拖动、拉伸、连线。
- **实时状态**：卡片实时显示思考中/执行工具/等待输入（思考球动效）；「运行中」看板自动聚合所有正在运行的任务。
...
整站统一毛玻璃体系（气泡、输入区、工具栏、面板同款材质），深浅主题分别调校，壁纸可透出。支持图片/视频壁纸，可拖拽位置、平铺、边缘填充；气泡的**透明度与磨砂强度**两条滑块随手可调，刷新后保留。

### 消息体验

- **消息钉**：把任意消息钉成置顶浮窗，全局拖拽、自由缩放，关键上下文边干活边看。
- **编辑回滚**：任意用户消息（首条、压缩后、连续）都可编辑回滚，语义与官方一致。
...
图形化管理全局与项目级 MCP 服务器：增删改查、启用/禁用、连通性测试。
