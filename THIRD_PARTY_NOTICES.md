...
但**依赖了部分非 MIT / 非开源协议的第三方组件**，使用与再分发时须遵守其各自许可条款。

---

## tldraw SDK（tldraw / @tldraw/sync / @tldraw/sync-core，5.x）

**看板功能（会话看板 / 画布）依赖 tldraw SDK。tldraw 不是开源协议，而是 source-available 的自有许可证。**

- 免费仅限**开发环境**（本机 loopback / localhost / 非 production）。
- 在**生产环境**（对外提供服务，含非 loopback 的内网 IP、公网部署）使用，必须持有有效 License Key：
