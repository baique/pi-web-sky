...
> 更新：2026-08-30 增量——游离态/锚定态点击行为、主动关闭回锚定（提交 5a22f80）
...
- **自由拖拽**：tab 条（含 chip）按下并移动即可拖动整个面板到任意位置，位置钳制在视口内（不越界）。
  ...
  - **游离态**：被拖离锚定位置（`dragPos !== null`）后视为“用户特意摆放”的浮动面板：
    点击面板外不再自动收起；只靠 × 按钮或触发按钮关闭。
  - **锚定态**：未被拖走时保留下拉语义——点击面板外自动收起（排除两个触发按钮的 toggle）。
  - **主动关闭回锚定**：无论哪种关闭路径（× / 触发按钮 toggle / 锚定态 outside-click），
    都重置 `dragPos` + `panelSize`，下次打开回到锚定位置与默认尺寸；
    拖走但未关闭（游离态保持打开）时位置/尺寸仍在组件 state 中保留。
  - 拖走后视觉切四圆角 + 完整边框（`.terminal-panel-dragged` 覆盖锚定态的单侧圆角）。
...
| 拖拽出视口 | 位置钳制在视口内，不会拖丢 |
| 缩放过小 | 钳制在最小 320×200 |
| 关闭重开面板 | 主动关闭（× / 触发按钮 / 锚定 outside-click）后回到锚定位置与默认尺寸；
  未关闭的游离面板位置/尺寸保留（组件 state 存活） |
| 游离态点击面板外 | 不收起（用户特意摆放）；锚定态点击面板外则自动收起 |
