...
> **⚠️ 已废弃（部分）**：任务卡设计已被 `2026-09-01-task-card-atomic-link.md`（原子-链接 + exec 线）取代。其中「展开态左表单右工作台」「专属执行会话占用语义」按新 spec §3.1 为准：任务卡不再内置工作台，执行会话以画布独立会话卡 + exec 线表达，occupied 概念已废除。调度器/状态机部分（§5/§6）不受影响。本文保留作历史参考。
