...
test("allows same-origin and non-browser API requests", async () => {
  ...
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "localhost:30143",
      origin: "http://localhost:30143",
      "sec-fetch-site": "same-origin",
    },
  })), true);
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: { host: "localhost:30143" },
  })), true);
});

test("allows LAN same-origin requests when Next.js uses an internal localhost URL", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const request = new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "192.168.32.7:30143",
      origin: "http://192.168.32.7:30143",
      "sec-fetch-site": "same-origin",
    },
  });
...
test("allows IPv6 and an explicitly configured hostname", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const ipv6 = new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "[::1]:30143",
      origin: "http://[::1]:30143",
  ...
  const configured = new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "pi-web.internal:30143",
      origin: "http://pi-web.internal:30143",
      "sec-fetch-site": "same-origin",
    },
  });
...
test("rejects cross-origin browser API requests", async () => {
  ...
  const post = new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "localhost:30143",
  ...
  const crossSiteGet = new Request("http://localhost:30143/api/sessions", {
    headers: { host: "localhost:30143", "sec-fetch-site": "cross-site" },
  });
...
test("does not globally trust opaque iframe or alternate loopback origins", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const previewUrl = "http://localhost:30143/api/files/tmp/test.docx?type=preview";
...
test("allows only user-initiated session export document navigations from a PWA", async () => {
  ...
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30143/api/sessions/session-id/export?inline=1",
    { headers: navigationHeaders },
  )), true);
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30143/api/sessions",
    { headers: navigationHeaders },
  )), false);
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30143/api/sessions/session-id/export?inline=1",
  ...
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30143/api/sessions/session-id/export?inline=1",
    { headers: { ...navigationHeaders, host: "attacker.example:30143" } },
  )), false);
});
...
test("rejects DNS rebinding even when browser headers say same-origin", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  const request = new Request("http://localhost:30143/api/skills/install", {
    method: "POST",
    headers: {
      host: "attacker.example:30143",
      origin: "http://attacker.example:30143",
      "sec-fetch-site": "same-origin",
...
test("rejects missing, malformed, and unconfigured Host headers", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30143/api/test")), false);
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30143/api/test", {
    headers: { host: "localhost@attacker.example:30143" },
  })), false);
  assert.equal(isApiRequestAllowed(new Request("http://localhost:30143/api/test", {
    headers: { host: "pi-web.internal:30143" },
  })), false);
});
...
test("allows same-origin requests when Chromium strips the port from Origin", async () => {
  ...
  const loopback = new Request("http://127.0.0.1:30143/api/agent/running", {
    headers: {
      host: "127.0.0.1:30143",
  ...
  const lan = new Request("http://192.168.32.7:30143/api/test", {
    method: "POST",
    headers: {
      host: "192.168.32.7:30143",
  ...
  const named = new Request("http://localhost:30143/api/test", {
    method: "POST",
    headers: {
      host: "localhost:30143",
...
test("still rejects when Origin hostname differs from Host even with port stripped", async () => {
  ...
  const crossName = new Request("http://localhost:30143/api/test", {
    headers: {
      host: "localhost:30143",
      origin: "http://127.0.0.1",
      "sec-fetch-site": "same-origin",
  ...
  const dnsRebind = new Request("http://localhost:30143/api/skills/install", {
    method: "POST",
    headers: {
      host: "attacker.example:30143",
      origin: "http://attacker.example",
