...
test("cadence fires only after N quiet turns while work is open", () => {
  ...
  for (let i = 0; i < REMINDER_INTERVAL - 1; i++) {
    onTurnStart(state);
    assert.equal(shouldInjectReminder(state, true), false);
  }

  onTurnStart(state);
  assert.equal(shouldInjectReminder(state, true), true, `第 ${REMINDER_INTERVAL} 轮触发`);
  // 注入后计时器推到当前轮 → 下一轮不再触发
  onTurnStart(state);
  assert.equal(shouldInjectReminder(state, true), false, "同周期只注入一次");
});
...
test("cadence fires even when the model calls no tools at all", () => {
  ...
  assert.equal(shouldInjectReminder(state, true), true);
});
...
test("using the todo tool resets cadence", () => {
  ...
  assert.equal(shouldInjectReminder(state, true), false, "刚用过 todo 工具，不提醒");
  ...
  assert.equal(shouldInjectReminder(other, true), true);
});

test("cadence stays silent when nothing is open", () => {
  const state = createReminderState();
  for (let i = 0; i < REMINDER_INTERVAL + 3; i++) onTurnStart(state);
  assert.equal(shouldInjectReminder(state, false), false);
});
