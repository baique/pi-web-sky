...
test("RPC wrapper avoids per-chunk idle and running-state maintenance", async () => {
  ...
  assert.doesNotMatch(source, /notifyRunningChange/);
});
...
test("reloading a session invalidates the models cache", async () => {
  ...
  // 0.86.0：forceEmptySystemPrompt 由 transformContext 投影实时生效，reload 不再需要手动重放。
  assert.doesNotMatch(reloadSource, /applyForcedEmptySystemPrompt/);
});

test("fork 建行失败不再静默（可见报错，不依赖扫描器兜底）", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const forkSource = source.slice(source.indexOf('case "fork"'), source.indexOf('case "navigate_tree"'));

  assert.match(forkSource, /console\.error\(\s*`\[pi-web\] fork 建行失败 session=\$\{newSessionId\}/);
  // 入板失败单独一条日志：不能被误报成「建行失败」。
  assert.match(forkSource, /console\.error\(\s*`\[pi-web\] fork 入板失败 source=\$\{sourceSessionId\}/);
  // 静默 catch 会让子会话以「无归属临时会话」出现在聊天区（V1 病灶）：fork 分支里不得再有裸 catch。
  assert.doesNotMatch(forkSource, /catch \{/);
});

test("新会话建行失败也不再静默（失败可见，不依赖扫描器兜底）", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const persistSource = source.slice(
    source.indexOf("async function persistNewSessionFile"),
    source.indexOf("export function getRpcSession"),
  );

  assert.match(persistSource, /console\.error\(\s*`\[pi-web\] 新会话建行失败 session=\$\{sessionId\}/);
  assert.doesNotMatch(persistSource, /catch \{/);
});
