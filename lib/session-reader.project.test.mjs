...
test("loadProjectSessions：first_message 空且文件有首条消息 → lazy 读文件补标题并写回 meta", async () => {
  freshDb();
  markIndexReady();
  ...
  try {
    const cwd = "/home/u/p";
    // 有消息但 meta 未同步（persist 建行冻结场景）→ lazy 应补
    ...
    // 真空会话：文件无消息 → 保持 no messages
    const emptyFile = join(projDir, "2026-01-02T00-00-00-000Z_b.jsonl");
    writeFileSync(emptyFile, [
      `{"type":"session","version":3,"id":"b","timestamp":"2026-01-02T00:00:00.000Z","cwd":"${cwd}"}`,
    ].join("\n") + "\n");
    seedSession({ id: "b", cwd, projectKey: cwd, path: emptyFile, firstMessage: null });

    const { loadProjectSessions } = await jiti.import("./session-reader.ts");
    const sessions = await loadProjectSessions(cwd);
    const byId = Object.fromEntries(sessions.map((s) => [s.id, s]));
    assert.equal(byId.a.firstMessage, "lazy 首条", "lazy 读文件补标题");
    assert.equal(byId.b.firstMessage, "(no messages)", "真空会话保持 no messages");
    const row = getDb().prepare("SELECT first_message FROM session_meta WHERE session_id='a'").get();
    assert.equal(row.first_message, "lazy 首条", "lazy 补读后写回 meta");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test("loadSessionDetailsFromMeta：任务列表同样 lazy 补标题并写回", async () => {
  freshDb();
  markIndexReady();
  ...
  try {
    ...
    assert.equal(sessions[0].firstMessage, "任务 lazy 首条", "任务列表 lazy 补标题");
    const row = getDb().prepare("SELECT first_message FROM session_meta WHERE session_id='a'").get();
    assert.equal(row.first_message, "任务 lazy 首条", "写回 meta");
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
