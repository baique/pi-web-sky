...
// Switch-back race: while a session is streaming, the SSE reconnect replays
// the partial message via `snapshot`. The mount-time state restore runs two
// fetches AFTER that connection is already open, and its `start` dispatch
// must NOT wipe the replayed streaming content — otherwise every subsequent
// text delta is dropped and the typewriter never resumes (the message only
// pops in whole at message_end).
test("start preserves an already-replayed streaming message (switch-back race)", () => {
  let state = snapshot(INITIAL_STREAMING_STATE, assistant([
    { type: "text", text: "partial reply" },
  ]));
  assert.equal(state.isStreaming, true);
  assert.equal(state.streamingMessage.content[0].text, "partial reply");

  // A late `start` (mount-time state restore completing after the SSE
  // snapshot replay) must keep the replayed content alive.
  state = streamReducer(state, { type: "start" });
  assert.equal(state.isStreaming, true);
  assert.equal(
    state.streamingMessage.content[0].text,
    "partial reply",
    "start must not wipe replayed streaming content",
  );

  // Deltas keep appending to the preserved message.
  state = delta(state, { type: "text_delta", contentIndex: 0, delta: " more" });
  assert.equal(state.streamingMessage.content[0].text, "partial reply more");
});

test("start on an empty stream still behaves as before", () => {
  const state = streamReducer(INITIAL_STREAMING_STATE, { type: "start" });
  assert.deepEqual(state, { isStreaming: true, streamingMessage: null });
});
