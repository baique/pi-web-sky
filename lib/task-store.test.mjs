...
test("setSessionTitle：写 title；老会话无行则 INSERT 首改，不影响归属/索引列", () => {
  freshDb();
  // 场景1：已有归属行（任务会话），改名写 title 保留 task_id
  seedSession("s-task", "task-1");
  setSessionTitle("s-task", "新名字");
  ...
  // 场景2：无行老会话首改 → INSERT，title 写入、task_id NULL
  setSessionTitle("s-orphan", "老会话改名");
  row = getDb().prepare("SELECT title, task_id, pinned FROM session_meta WHERE session_id='s-orphan'").get();
  assert.equal(row.title, "老会话改名");
  assert.equal(row.task_id, null);
  assert.equal(row.pinned, 0);
  // 场景3：再次改名覆盖
  setSessionTitle("s-orphan", "再改");
  assert.equal(getDb().prepare("SELECT title FROM session_meta WHERE session_id='s-orphan'").get().title, "再改");
});
