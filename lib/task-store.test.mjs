...
const agentDir = mkdtempSync(join(tmpdir(), "pi-web-task-store-"));
...
const { createTask, deleteTask, listTasks, updateTask, taskForSession, assignSessionToTask, reorderTasks, setSessionTitle } = await jiti.import("./task-store.ts");
const { getBoard, getOrCreateTaskBoard, createBoard } = await jiti.import("./board-store.ts");
...
test("assignSessionToTask attaches a session atomically and bumps the task's updated", async () => {
  freshDb();
  const a = createTask("p", "A");
  seedSession("s1", null); // temp
  seedSession("s2", a.id); // already a member

  assert.equal(await assignSessionToTask("s1", a.id), true);
  assert.equal(taskForSession("s1"), a.id);
  // 重复归属幂等：已是成员再归一次仍成功且不重复。
  assert.equal(await assignSessionToTask("s2", a.id), true);
...
test("assignSessionToTask returns false for a missing task without writing anything", async () => {
  freshDb();
  seedSession("s1", null);
  assert.equal(await assignSessionToTask("s1", "no-such-task"), false);
  assert.equal(taskForSession("s1"), null);
  assert.equal(listTasks("p").length, 0);
});
...
test("assignSessionToTask moves a session out of its previous task", async () => {
  freshDb();
  const a = createTask("p", "A");
  const b = createTask("p", "B");
  seedSession("s1", null);
  await assignSessionToTask("s1", a.id);
  await assignSessionToTask("s1", b.id); // 移动到 B
  assert.equal(taskForSession("s1"), b.id);
  const taskA = listTasks("p").find((t) => t.id === a.id);
  assert.deepEqual(taskA.sessionIds, []);
});

test("updateTask 不再为无行会话插缺列局部行（加侧只 UPDATE）", () => {
  freshDb();
  const a = createTask("p", "A");
  updateTask(a.id, { sessionIds: ["ghost-no-row"] });
  // 行由路由层 updateTask 之前调 ensureSessionRows 建（那里才能 await 文件 I/O）。
  assert.equal(
    getDb().prepare("SELECT COUNT(*) c FROM session_meta WHERE session_id='ghost-no-row'").get().c,
    0,
  );
  assert.deepEqual(listTasks("p")[0].sessionIds, []);
});

test("updateTask 对已有行的成员照常写归属（加侧 UPDATE 不丢写）", () => {
  freshDb();
  const a = createTask("p", "A");
  seedSession("s1", null);
  updateTask(a.id, { sessionIds: ["s1"] });
...
test("deleteTask cascades to its task board (nodes/edges/view gone)", () => {
  ...
  // 写入 tldraw 遗留表节点（deleteTask 级联删看板时需清空）
  const ts = Date.now();
  const db = getDb();
  db.prepare("INSERT INTO board_nodes (id, board_id, kind, ref_id, x, y, w, h, expanded, props, created, updated) VALUES (?, ?, 'session', ?, ?, ?, ?, ?, ?, ?, ?, ?)")
    .run("n1", board.id, "s1", 0, 0, 100, 100, 0, "{}", ts, ts);
  deleteTask(a.id);
  assert.equal(getBoard(board.id), undefined);
  assert.equal(db.prepare("SELECT COUNT(*) c FROM board_nodes WHERE board_id = ?").get(board.id).c, 0);
  // 手动看板不受影响
...
test("setSessionTitle：已有行只改名（不覆盖归属/索引列）", async () => {
  freshDb();
  // 场景1：已有归属行（任务会话），改名写 title 保留 task_id
  seedSession("s-task", "task-1");
  await setSessionTitle("s-task", "新名字");
  let row = getDb().prepare("SELECT title, task_id FROM session_meta WHERE session_id='s-task'").get();
  assert.equal(row.title, "新名字");
  assert.equal(row.task_id, "task-1", "不覆盖已有归属");
  // 场景2：再次改名覆盖
  await setSessionTitle("s-task", "再改");
  assert.equal(getDb().prepare("SELECT title FROM session_meta WHERE session_id='s-task'").get().title, "再改");
  // 场景3（无行 + 磁盘无文件 → 不写缺列局部行）在 task-store.partial-row.test.mjs 覆盖。
  assert.equal(getDb().prepare("SELECT COUNT(*) c FROM session_meta").get().c, 1);
});
