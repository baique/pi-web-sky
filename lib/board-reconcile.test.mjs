...
// 隔离：归属写入现在会对「库里无行」的会话解析会话文件，临时 agentDir 保证
// 测试绝不扫开发机真实的 ~/.pi/agent/sessions。
const agentDir = mkdtempSync(join(tmpdir(), "pi-web-board-reconcile-"));
...
/** 库里已有该会话的行且已归属本任务（生产链路：行由创建链路/扫描器建、归属由
 *  assignSessionToTask 写；reconcile 只消费「行 + 归属」，这里一次插齐）。 */
function seedAssignedSession(sid, taskId) {
  const ts = Date.now();
  getDb().prepare(
    "INSERT INTO session_meta (session_id, task_id, updated, pinned, path, cwd, project_key, created, modified) " +
      "VALUES (?, ?, ?, 0, ?, ?, ?, ?, ?)",
  ).run(sid, taskId, ts, `/sessions/proj/${sid}.jsonl`, "/proj", PROJECT, ts, ts);
}
...
test("窗口期 reconcile 不补同 sid 占位卡（新建会话指定 ID 场景）", async () => {
  ...
  // 服务端已落库（行由创建链路建，归属在 /api/agent/new 中先于前端转正写入）
  const sid = "abc-123";
  seedAssignedSession(sid, task.id);
...
test("占位卡不影响其他会话补卡（6461a61 语义保留）", async () => {
  ...
  seedAssignedSession(realSid, task.id);
  ...
  seedAssignedSession(execSid, task.id);

  const { reconcileBoard } = await jiti.import("./board-reconcile.ts");
  await reconcileBoard(board.id);

  // 业务表会话都没有对应画布卡 → reconcile 补齐 realSid/execSid 会话卡
  ...
  // 任务卡是用户画布内容：reconcile 绝不从业务表反向补卡
  const taskNode = [...mockState.nodeSets.values()].find((n) => n.type === "task-card");
  assert.equal(taskNode, undefined, "reconcile 不补任务卡节点");
  // 画布无任务卡锚点 → 不建 exec 线（exec 线要求宿主卡已在画布）
  const execEdge = [...mockState.edgeSets.values()].find((e) => e.data?.execLink);
  assert.equal(execEdge, undefined, "无任务卡锚点不建 exec 线");
});

test("普通看板任务卡派发后：补执行会话卡，无任务卡锚点不建 exec 线（不入板 bug 回归）", async () => {
  ...
  // 普通看板上的任务已派发（sessionId 已落表），但任务卡节点尚未拖入画布
...
test("任务卡节点已在画布：补执行会话卡并建 exec 线（锚点派生语义）", async () => {
  const { createBoard } = await jiti.import("./board-store.ts");
  const board = createBoard(PROJECT, "锚点看板");

  const execSid = "exec-anchor-line";
  const card = createCard({ boardId: board.id, projectKey: PROJECT, name: "锚点任务" });
  updateCard(card.id, { sessionId: execSid });
  // 任务卡节点已在画布（用户布局）
  mockState.nodeSets.set(`task-${card.id}`, {
    id: `task-${card.id}`,
    type: "task-card",
  ...
  // 执行会话卡补出 + exec 线建立（source=任务卡节点，target=执行会话卡节点）
  assert.ok(mockState.nodeSets.has(`session-${execSid}`), "执行会话卡应补出");
  const execEdge = [...mockState.edgeSets.values()].find((e) => e.data?.execLink);
  assert.ok(execEdge, "有任务卡锚点应建 exec 线");
  assert.equal(execEdge.source, `task-${card.id}`);
  assert.equal(execEdge.target, `session-${execSid}`);
});

test("任务执行会话卡补卡：锚定任务卡右侧、y 齐平（不落 4 列布局）", async () => {
  const { createBoard } = await jiti.import("./board-store.ts");
  const board = createBoard(PROJECT, "锚定看板");

  // 用户已把任务卡摆在画布上（x=200, y=300，宽 380）
  const execSid = "exec-anchor-session";
  const card = createCard({ boardId: board.id, projectKey: PROJECT, name: "锚定任务" });
  updateCard(card.id, { sessionId: execSid });
  mockState.nodeSets.set(`task-${card.id}`, {
    id: `task-${card.id}`,
    type: "task-card",
  ...
  const node = mockState.nodeSets.get(`session-${execSid}`);
  assert.ok(node, "执行会话卡应补出");
  // 右侧首选：x = 任务卡.x + 任务卡宽 + 24；y 与任务卡齐平
  assert.equal(node.position.x, 200 + 380 + 24);
  assert.equal(node.position.y, 300);
});
...
test("拖入任务看板的会话：无 session_meta 归属 → 判孤儿删；有归属 → 存活不补卡", async () => {
  ...
  seedAssignedSession(assignedSid, task.id);
...
test("侧栏拖会话到看板行：落卡到空位、重复拖同 sid 不动", async () => {
  const { addSessionCardToBoard } = await jiti.import("./board-reconcile.ts");

  // 已占 4 列首列（60,60）→ 新卡应落第二列（60 + 340 + 24）
  mockState.nodeSets.set("session-占用", cardNode("session-占用", "占用", {}));

  assert.equal(await addSessionCardToBoard("board-x", "sid-new", "拖来的标题"), true);
  const node = mockState.nodeSets.get("session-sid-new");
  assert.ok(node, "落卡存在");
  assert.equal(node.type, "session-card");
  assert.equal(node.data.title, "拖来的标题", "标题随拖拽源带入");
  assert.equal(node.data.cwd, "", "cwd 必须空（非空 = 新建会话占位卡语义）");
  assert.equal(node.position.x, 60 + 340 + 24, "不与已有卡重叠");

  // 再拖同一会话：不重落（保留用户布局/展开态），也不改标题
  assert.equal(await addSessionCardToBoard("board-x", "sid-new", "新标题"), false);
  assert.equal(mockState.nodeSets.get("session-sid-new").data.title, "拖来的标题");
  assert.equal(mockState.nodeSets.size, 2, "只有一张新卡");
});
