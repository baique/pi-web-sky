...
test("v10 database upgrades to v11 (session_meta index columns + project index)", () => {
  const db = new DatabaseSync(":memory:");
  // 模拟 v10 时代老库：session_meta 只有任务/置顶旁路列，无索引列。
  db.exec(`
    CREATE TABLE tasks (
      id TEXT PRIMARY KEY,
      project_key TEXT NOT NULL,
    ...
    INSERT INTO session_meta (session_id, task_id, updated, pinned)
      VALUES ('s-kept', 'task-1', 100, 1);
  `);
  ...
  assert.equal(db.prepare("PRAGMA user_version").get().user_version, SCHEMA_VERSION);
  const cols = db.prepare("SELECT name FROM pragma_table_info('session_meta')").all().map((r) => r.name);
  for (const c of ["path", "cwd", "project_key", "title", "first_message", "parent_id", "created", "modified"]) {
    assert.ok(cols.includes(c), `session_meta.${c} added by v11`);
  }
  // 索引存在
  const idx = db.prepare("SELECT name FROM sqlite_master WHERE type='index' AND name='idx_meta_project_modified'").get();
  assert.ok(idx, "idx_meta_project_modified created by v11");
  // 老数据保留
  const row = db.prepare("SELECT session_id, task_id, pinned FROM session_meta WHERE session_id='s-kept'").get();
  assert.equal(row.session_id, "s-kept");
...
test("schema v12：老库升级后 session_meta 有 last_reply 列，新库直接具备", () => {
  ...
  old.exec(
    "CREATE TABLE session_meta (session_id TEXT PRIMARY KEY, task_id TEXT, updated INTEGER NOT NULL, pinned INTEGER NOT NULL DEFAULT 0, path TEXT, cwd TEXT, project_key TEXT, title TEXT, first_message TEXT, parent_id TEXT, created INTEGER, modified INTEGER)",
  );
  old.prepare(
    "INSERT INTO session_meta (session_id, updated, modified) VALUES ('s-old', 1, 2)",
  ).run();

  initSchema(old);

  const cols = old.prepare("PRAGMA table_info(session_meta)").all().map((c) => c.name);
  assert.ok(cols.includes("last_reply"), "v12 adds session_meta.last_reply");
  assert.equal(old.prepare("PRAGMA user_version").get().user_version, 12);
  // 老行保留，新列默认 NULL（"还没有最后一条消息"）
  const kept = old.prepare("SELECT session_id, last_reply FROM session_meta WHERE session_id='s-old'").get();
  assert.equal(kept.session_id, "s-old");
  assert.equal(kept.last_reply, null);
  // 新库第一次建表就该有该列（老库升级与新库同构）
  const fresh = memoryDb();
  const freshCols = fresh.prepare("PRAGMA table_info(session_meta)").all().map((c) => c.name);
  assert.ok(freshCols.includes("last_reply"));
  assert.equal(fresh.prepare("PRAGMA user_version").get().user_version, 12);
});
