 ...
 * - auto-clear：全部完成后再过 2 轮自动清空并复位 nextId
 * - 节奏提醒：连续 4 轮没用 todo 工具且仍有未完成项时，在 `context` 事件里注入
 *   **瞬时**提醒（不落盘、不进会话文件、聊天界面不出现卡片）
 *
...
export function noteToolResult(state: ReminderState, toolName: string): void {
  if (toolName === TOOL_NAME) state.lastTodoToolUseTurn = state.currentTurn;
}

/**
 * `context` 事件时求值：距上次 todo 工具使用已过 REMINDER_INTERVAL 轮且有未完成项 → 注入一次。
 *
 * 求值点必须在 `context`（每轮 LLM 请求前都到），不能只在 `tool_result` 里 arm：
 * 纯文本轮（模型没调任何工具）不发 `tool_result`，那样的回合永远不会触发提醒。
 * 注入后立即把 lastTodoToolUseTurn 推到当前轮 —— 这保证一个周期只提醒一次。
 */
export function shouldInjectReminder(state: ReminderState, hasOpenWork: boolean): boolean {
  ...
  return {
    ...
    factory: (pi: ExtensionAPI) => {
      ...
      pi.on("context", async (event) => {
        if (!shouldInjectReminder(reminder, hasOpenTodos(state.todos))) return undefined;
