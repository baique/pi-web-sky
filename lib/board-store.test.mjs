...
test("schema version reaches latest with board tables", () => {
  ...
  assert.equal(SCHEMA_VERSION, 12); // v12: session_meta.last_reply
...
test("removeSessionFromBoards: 删会话清任务卡 session_id + 待答队列，幂等", () => {
  ...
  updateCard(c2.id, { sessionId: "sess-x", execStatus: "done" });
  ...
  // 状态结算：非终态 → 「放弃」（否则删会话后卡永久停在进行中，占住并发位）
  assert.equal(getCard(c.id)?.execStatus, "abandoned");
  // 终态 → 不覆盖已有结论
  assert.equal(getCard(c2.id)?.execStatus, "done");
  // 幂等：重复调用无副作用
