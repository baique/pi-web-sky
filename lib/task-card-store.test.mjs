...
const { getCard, listCards, listCardsByIds, createCard, updateCard, deleteCard, unbindCardSession, addLink, removeLink, replaceLinks, listLinks, listDispatchableCards, countRunningDispatched, acquireDispatchToken, clearDispatchToken, createQuestion, listQuestions, listCardQuestions, answerQuestion, countPendingQuestions, listAnswerableQuestions } = await jiti.import("./task-card-store.ts");
...
test("createCard 默认值与必填校验", () => {
  ...
  assert.equal(c.readyStatus, "todo"); // 派发语义：建卡即 todo（可调度）
...
test("listCardsByIds 按传入顺序返回、跳过不存在、空数组返回空", () => {
  freshDb();
  const b = createBoard("proj-a", "看板A");
  const c1 = createCard({ boardId: b.id, projectKey: "proj-a", name: "一" });
  createCard({ boardId: b.id, projectKey: "proj-a", name: "二" });
  const c3 = createCard({ boardId: b.id, projectKey: "proj-a", name: "三" });
  updateCard(c1.id, { execStatus: "failed" }); // 任意状态都返回（含非活跃态）

  // 传入乱序 + 一个不存在 id
  const got = listCardsByIds([c3.id, "no-such-id", c1.id]);
  assert.deepEqual(got.map((c) => c.id), [c3.id, c1.id]);
  assert.equal(got[1].execStatus, "failed", "failed 卡也应按 id 查回（状态分离后可见卡全量拉取）");
  assert.deepEqual(listCardsByIds([]), []);
});
...
test("deleteCard 级联删 links/questions 与 taskcard node/边", () => {
  ...
  // 画布节点（kind=taskcard, refId=cardId）与会话卡节点（直接写 tldraw 遗留表）
  const db0 = getDb();
  const ts0 = Date.now();
  const insNode = (id, kind, refId, x) =>
    db0.prepare("INSERT INTO board_nodes (id, board_id, kind, ref_id, x, y, w, h, expanded, props, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 0, '{}', ?, ?)")
      .run(id, b.id, kind, refId, x, 10, 220, 120, ts0, ts0);
  insNode("n1", "taskcard", c1.id, 10);
  insNode("n2", "taskcard", c2.id, 300);
  insNode("nsess", "session", "sess-x", 0);
  ...
  db.prepare("INSERT INTO board_edges (id, board_id, from_id, to_id, label, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .run("e1", b.id, "n1", "n2", "prerequisite", Date.now(), Date.now());
  ...
  const nodeRows = db.prepare("SELECT * FROM board_nodes WHERE id = ?").get("n1");
  ...
  const otherNode = db.prepare("SELECT * FROM board_nodes WHERE id = ?").get("n2");
...
test("listDispatchableCards 前置过滤 + 就绪/重试条件", () => {
  ...
  // 建卡默认 todo；pre（前置）和 draft 显式回退草稿——测前置未完成/draft 不就绪
  updateCard(pre.id, { readyStatus: "draft" });
  updateCard(draft.id, { readyStatus: "draft" });
  // overRetry/retryable 保持 todo（测重试上限）
...
test("listDispatchableCards 按优先级降序派发（高→中→低，同优先级按编号）", () => {
  freshDb();
  const b = createBoard("proj-a", "看板A");
  const low = createCard({ boardId: b.id, projectKey: "proj-a", name: "低", priority: -1 });
  const high = createCard({ boardId: b.id, projectKey: "proj-a", name: "高", priority: 1 });
  const mid = createCard({ boardId: b.id, projectKey: "proj-a", name: "中", priority: 0 });
  const retryHigh = createCard({ boardId: b.id, projectKey: "proj-a", name: "重试高", priority: 1 });
  for (const c of [low, high, mid, retryHigh]) updateCard(c.id, { readyStatus: "todo" });
  updateCard(retryHigh.id, { execStatus: "failed", retryCount: 1, maxRetries: 3 });
  const order = listDispatchableCards().map((c) => c.priority);
  // 严格降序：高(1) 先于 中(0) 先于 低(-1)；同优先级（两个 1）按编号升序
  assert.deepEqual(order, [1, 1, 0, -1]);
});

test("acquireDispatchToken 原子互斥：抢到后卡不可再派发，释放后可再抢", () => {
  freshDb();
  const b = createBoard("proj-a", "看板A");
  const card = createCard({ boardId: b.id, projectKey: "proj-a", name: "派发卡" });
  updateCard(card.id, { readyStatus: "todo" });

  // 抢锁成功 → 卡从可派发列表消失
  assert.ok(acquireDispatchToken(card.id, "token-a"));
  assert.ok(!listDispatchableCards().some((c) => c.id === card.id));
  // 第二个实例抢同一张卡失败（token 已占用）
  assert.ok(!acquireDispatchToken(card.id, "token-b"));
  // 错误 token 释放无效（防误清他人锁）
  clearDispatchToken(card.id, "token-b");
  assert.ok(!listDispatchableCards().some((c) => c.id === card.id));
  // 正确 token 释放 → 重新可派发、可再抢
  clearDispatchToken(card.id, "token-a");
  assert.ok(listDispatchableCards().some((c) => c.id === card.id));
  assert.ok(acquireDispatchToken(card.id, "token-c"));
});
...
test("问答：建/列/答/计数 + listAnswerableQuestions camelCase 回归", () => {
  ...
  assert.equal(listCardQuestions(c2.id)[0].id, q2.id);
...
test("unbindCardSession：解绑并结算状态（普通看板移除执行会话卡）", () => {
  freshDb();
  const b = createBoard("proj-unbind", "看板U");
  const mk = (tag, execStatus) => {
    const c = createCard({ boardId: b.id, projectKey: "proj-unbind", name: `卡-${tag}` });
    updateCard(c.id, { sessionId: `sess-${tag}`, execStatus });
    return c;
  };

  // 非终态：解绑 + 记「放弃」（否则卡停在执行中 / 等你回答 / 待审核）
  for (const status of ["not_started", "running", "waiting_reply", "review"]) {
    const c = mk(status, status);
    unbindCardSession(`sess-${status}`);
    assert.equal(getCard(c.id)?.sessionId, null, `${status}: 绑定应清空`);
    assert.equal(getCard(c.id)?.execStatus, "abandoned", `${status}: 应记为放弃`);
  }

  // 终态：只解绑，不覆盖已有结论
  for (const status of ["done", "failed", "abandoned"]) {
    const c = mk(`t-${status}`, status);
    unbindCardSession(`sess-t-${status}`);
    assert.equal(getCard(c.id)?.sessionId, null, `${status}: 绑定应清空`);
    assert.equal(getCard(c.id)?.execStatus, status, `${status}: 结论不应被覆盖`);
  }

  // 无绑定（普通会话卡 / 外部会话）：无操作
  unbindCardSession("sess-无绑定");
});
