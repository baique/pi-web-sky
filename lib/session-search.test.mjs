...
test("searchSessions matches Chinese substring, 2-char LIKE fallback, English, titleMatch", async () => {
  ...
  try {
    ...
    const { results: m1 } = await searchSessions("登录接口", 10, sessions);
    ...
    const { results: mTitle } = await searchSessions("登录重构", 10, sessions);
    ...
    const { results: m2 } = await searchSessions("登录", 10, sessions);
    assert.deepEqual(m2.map((r) => r.session.id), ["s1"]);

    // English word.
    const { results: m3 } = await searchSessions("fts", 10, sessions);
    assert.deepEqual(m3.map((r) => r.session.id), ["s2"]);
    assert.equal(m3[0].titleMatch, true);

    // No results.
    const { results: none } = await searchSessions("不存在的词xyz", 10, sessions);
    assert.equal(none.length, 0);

    // Quoted/special chars must not throw.
    const { results: safe } = await searchSessions('a"b', 10, sessions);
...
test("searchSessions 多关键字空格分隔 AND（FTS 与 LIKE 回退）", async () => {
  freshDb();
  const dir = fixtureDir({ "s1.jsonl": SAMPLE_LINES });
  try {
    const s1 = makeSession("s1", join(dir, "s1.jsonl"), { name: "登录重构" });
    ...
    // FTS 路径（两词均 ≥3 字符，且都在 s1 body 命中）："登录接口 token" → s1
    const { results: fts } = await searchSessions("登录接口 token", 10, sessions);
    assert.deepEqual(fts.map((r) => r.session.id), ["s1"], "FTS 多词 AND");

    // LIKE 回退（含 2 字符词）title 命中："登录 重构" → s1（title="登录重构"）
    const { results: likeTitle } = await searchSessions("登录 重构", 10, sessions);
    assert.deepEqual(likeTitle.map((r) => r.session.id), ["s1"], "LIKE 回退多词 AND title");
    assert.equal(likeTitle[0].titleMatch, true);

    // LIKE 回退 body 场景："登录 token" → s1（body 含两词）
    const { results: likeBody } = await searchSessions("登录 token", 10, sessions);
    assert.deepEqual(likeBody.map((r) => r.session.id), ["s1"], "LIKE 回退多词 AND body");

    // 缺任一词不命中：s1 无 "不存在词xyz"
    const { results: miss } = await searchSessions("登录接口 不存在词xyz", 10, sessions);
    assert.equal(miss.length, 0, "缺一词不命中");

    // 多空格/首尾空格容忍
    const { results: spaced } = await searchSessions("  登录  token  ", 10, sessions);
    assert.deepEqual(spaced.map((r) => r.session.id), ["s1"], "多空格容忍");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("searchSessions sessionIds 过滤：仅搜索指定会话", async () => {
  freshDb();
  const dir = fixtureDir({ "s1.jsonl": SAMPLE_LINES });
  try {
    const s1 = makeSession("s1", join(dir, "s1.jsonl"), { name: "登录重构" });
    ...
    // 过滤到 s2：s2 无正文、标题不命中的词 → 空
    const { results: only2 } = await searchSessions("登录接口", 10, sessions, ["s2"]);
    assert.equal(only2.length, 0, "sessionIds 过滤后不含 s1");

    // 过滤到 s1：正文命中保留
    const { results: only1 } = await searchSessions("登录接口", 10, sessions, ["s1"]);
    assert.deepEqual(only1.map((r) => r.session.id), ["s1"], "sessionIds=[s1] 命中");

    // 多词 FTS 路径 + 过滤组合
    const { results: fts } = await searchSessions("登录接口 token", 10, sessions, ["s1"]);
    assert.deepEqual(fts.map((r) => r.session.id), ["s1"], "FTS 多词 + sessionIds 过滤");

    // 两词都命中过滤集但命中词只在过滤外的会话 → 空
    const { results: none } = await searchSessions("登录接口", 10, sessions, ["s2"]);
    assert.equal(none.length, 0, "命中词不在过滤集内 → 空");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
