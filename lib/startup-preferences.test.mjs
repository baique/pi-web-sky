...
test("deletes the thinking default when the effective level is invalid (auto)", async () => {
  await withSettings(async ({ settings, settingsPath }) => {
    ...
    await persistExplicitStartupPreferences(
      settings,
      { thinkingLevel: "auto" },
      { thinkingLevel: "auto", supportsThinking: true },
    );

    const saved = JSON.parse(await readFile(settingsPath, "utf8"));
    assert.equal("defaultThinkingLevel" in saved, false);
    assert.equal(settings.getDefaultThinkingLevel(), undefined);
  });
});
