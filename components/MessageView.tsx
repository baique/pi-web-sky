...
function UserMessageView({ message, cwd, onOpenFile, entryId, onFork, forking, onNavigate, parentEntryId, isLeafEntry, onEditContent, onPin, bare }: {
  ...
  const skillData = skillExpansionToCommand(content);
  const commandText = skillData?.command;
  ...
  const commandArgs = commandText && commandSeparator !== -1
    ...
    : "";
  const skillBody = skillData?.skillBody;
        ...
        >
          {commandText ? (
            <div style={{ display: "flex", flexDirection: "column", gap: 6, minWidth: 0 }}>
              ...
              {expanded && skillBody && (
                <div style={{ margin: "2px 0", borderLeft: "2px solid var(--bubble-hairline)", paddingLeft: 10, fontSize: 12 }}>
                  <MarkdownBody className="markdown-skill-body" cwd={cwd} onOpenFile={onOpenFile}>{skillBody}</MarkdownBody>
                </div>
              )}
              {commandArgs && (
                <span style={{
