...
export function AppShell() {
 ...
 useEffect(() => {
  ...
  // 会话归属任务名（实时）：null=无任务/未拉取，string=任务名。面板打开时拉取，
  // 拖拽移入/移出任务后不依赖 selectedSession 的过期快照。
  ...
  useEffect(() => {
    if (activeTopPanel !== "session" || !selectedSession?.id) {
      setLiveTaskName(null);
      return;
    }
    let cancelled = false;
    setLiveTaskName(null);
    fetch(`/api/sessions/${encodeURIComponent(selectedSession.id)}`, { cache: "no-store" })
      .then((r) => (r.ok ? r.json() as Promise<{ info?: { taskName?: string } | null }> : null))
      .then((d) => {
                ...
                <div className="session-info-popover glass-top-panel" style={{
                  ...
                  {sessionStats ? (() => {
                    ...
                    const sessionRows = [
                       ...
                       ...(liveTaskName ? [{ label: translate("session.task"), value: liveTaskName, copyField: null }] : []),
