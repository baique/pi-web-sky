 ...
 * - setHighlight(id)：定位时调用；自动在 HIGHLIGHT_MS 后清除（渐隐结束即消失）
 */

import { createContext, useCallback, useContext, useEffect, useRef, useState } from "react";
import type { ReactNode } from "react";

/** 高亮描边持续时长（ms，与 CSS 动画 board-search-glow 时长 1.8s 保持一致） */
const HIGHLIGHT_MS = 1800;

/** 命中高亮描边：accent 外圈 + 泛光（渐隐由 board-search-glow keyframes 控制） */
...
export function BoardSearchProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <BoardSearchContext.Provider value={{ highlightId, setHighlight }}>
      {children}
...
export function useBoardSearch(): BoardSearchContextValue {
  ...
  if (!ctx) return { highlightId: null, setHighlight: () => {} };
  return ctx;
}
