...
> 改任务卡 / 任务卡数据模型 / 依赖线 / 任务卡表单前阅读。spec 见 `.agent/spec/2026-08-31-task-card-scheduler.md`，S1 计划见 `.agent/plan/2026-08-31-task-cards-s1.md`。
...
**形态**：常态 = 编辑表单栏（340px，左侧常驻，直接可输入）；展开 = 右侧追加执行会话工作台（900px）。双击/「展开工作台」按钮切换。从工具栏**拖出创建**（像便笺，`ToolbarItem tool="task-card"`）。
...
task_cards
  ...
  max_retries(默认3) / retry_count / session_id(专属执行会话)
  created / updated
