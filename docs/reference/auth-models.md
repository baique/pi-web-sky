# 认证、模型与 enabledModels

> 改 provider 列表 / 登录 / models.json / 模型选择 / 思考级别前阅读。

## Auth and model config

- `ModelsConfig` combines models from `~/.pi/agent/models.json` with provider auth status from pi's `ModelRuntime` (`modelRuntime.getProviderAuthStatus()`; `lib/provider-listing-runtime.ts` adapts it).
- Provider listing is capability-driven, never id-driven: `lib/provider-listing.ts` decides membership from `auth.apiKey.login` / `auth.oauth` plus the stored credential type, so dual-auth providers (anthropic, github-copilot, kimi-coding, openrouter, radius, xai today — which providers declare both changes between SDK releases, so never assume it from an id) appear exactly once and never fall through both lists (#309). `lib/provider-listing-runtime.ts` adapts `ModelRuntime` to those pure helpers.
...
- API-key routes store and remove keys through `lib/provider-credential-store.ts` (`storeProviderCredential` / `removeStoredCredentialIfType`, same file lock as pi's auth storage). Status endpoints must never return the raw key.
