...
- **标题（= 展示层的 `title || first_message`）与首条消息**：`first_message` 的写入优先级是「**真首条消息 > prompt 提交原文**，已有非空值永不被覆盖」。三个写入者：① RPC prompt 分支（`lib/rpc-manager.ts` 的 `await preflight` 之后）——**仅当 transcript 里还没有用户消息**时，用提交原文写（trim + 截断 300）；这是默认标题能「一发消息就出现」的唯一原因——**不要改回「只等 agent_settled」**，长首轮（几分钟~几十分钟）期间刷新页面/换客户端/看板卡上标题会一直是 `(no messages)`。② `agent_settled`——用内存 entries 的真首条用户消息兜底（扩展注入/外部来源的起点不是 RPC prompt）。③ 30s 扫描器——文件头 48KB 内能翻到首条用户消息时补空/收敛。两个坑：**不能拿 `agent_start` 当写入点**（实测那一刻用户消息还没进 entries，拿不到文本）；**不能对已有历史的会话写提交原文**（会把最新一条消息钉成永久错值，且 fill-empty 之后没人能纠正）。prompt 分支的库写必须包 try/catch：`send()` 抛出会让路由把「已接受的 prompt」报成 `prompt_rejected`（客户端回填草稿、诱导重发）。
- **列表变更信号（`lib/session-list-signal.ts`）**：任何影响列表的写路径（改名/pin/新建/删除/归属/事件写库/扫描器入库，都经 `invalidateSessionListCache()`）自增一次代次，`GET /api/agent/running` 与 `GET /api/sessions`（两份响应各自带）都带出 `listGeneration`；侧栏 2.5s 轮询用 `shouldRefreshForListGeneration`（`components/session-sidebar-list.ts` 纯函数）比对——列表响应把「我这份列表对应的代次」当种子存下，轮询见到不同代次就 bump `refreshKey`（防抖全量重拉会话/任务）。这是「服务端写进去了但前端不重拉」的唯一出路——侧栏只在挂载/切项目/本地动作时重拉，没有它标题/首条消息/外部（CLI）新建的会话会一直躺在库里不显示。**代次是进程内计数器**：多实例共用一个 `pi-web.db` 时只覆盖与 DB 同进程的写入。
...
- **聊天区时间分组（2026-09）**：前端把已排好序的根会话切成今天/昨天/本周/近一月/更久之前五段，只插小角标不改排序（`lib/session-time-group.ts` 纯分类函数：日历天判今天/昨天、周一为周首、近一月 = 滚动 30 天）。**今天段不打角标**（这一段总在列表最上面，标出来只是噪音），**置顶段不分组**，越过置顶分隔线后重新起头；fork 子会话跟着根所在段。
