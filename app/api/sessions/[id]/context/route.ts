...
import { resolveSessionPath, buildSessionContext, hasOlderHistory } from "@/lib/session-reader";
...
export async function GET(
  ...
  try {
    ...
    const hasMore = hasOlderHistory(sm.getEntries() as never, before ?? leafId ?? null, tail, Boolean(before));
    return NextResponse.json({ context, tail, before: before ?? null, hasMore });
  } catch (error) {
